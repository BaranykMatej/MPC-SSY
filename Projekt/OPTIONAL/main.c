...
bool TMP102Sensor_Init(TMP102Sensor* sensor) {
	if (TWI_MasterInit() != TWI_SUCC) {
		printf("TWI initialization failed.\n");
		return false;
	}
	if (TMP102_Init() != TWI_SUCC) {
		printf("TMP102 initialization failed.\n");
		return false;
	}
	return true;
}
...
bool TMP102Sensor_ReadTemperature(TMP102Sensor* sensor) {
	sensor->status = TMP102_StartOneshotMeasurement();
	if (sensor->status != TWI_SUCC) {
		printf("Failed to start measurement.\n");
		return false;
	}

	sensor->status = TMP102_GetTemperature(&(sensor->temperature), true);
	return (sensor->status == TWI_SUCC);
}
